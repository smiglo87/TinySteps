...
public class IPCycler : MonoBehaviour {
	...
	public float 		spacing = 30f,	// The horizontal or vertical space between cycled transforms ( positive values only )
						dragScale = 1f, //Sets UIScrollView.scale according to the direction of the cycler. If 0, UIScrollView.scale will be left as is.
	...
	public UIScrollView 	_draggablePanel;
	[HideInInspector]
	public UIDragScrollView	dragPanelContents;
	[HideInInspector]
	public IPUserInteraction	userInteraction;
	UIPanel 				_panel;
	UICenterOnChildManual 	_uiCenterOnChildManual;
	...
	public void Init()
	{
		...
		if ( _pickerCollider != null )
		{
			( dragPanelContents = _pickerCollider.gameObject.AddComponent ( typeof ( UIDragScrollView ) ) as UIDragScrollView ).draggablePanel = _draggablePanel;
		...
		_uiCenterOnChildManual = gameObject.AddComponent ( typeof ( UICenterOnChildManual ) ) as UICenterOnChildManual;
		...
		if ( dragScale != 0f )
		{
			_draggablePanel.scale = _isHorizontal ? new Vector3 ( dragScale, 0f, 0f ) : new Vector3 ( 0f, dragScale, 0f );
		}
